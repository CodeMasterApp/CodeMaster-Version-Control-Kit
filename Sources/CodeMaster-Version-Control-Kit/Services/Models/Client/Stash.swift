...
public struct Stash {
    ...
    private let editorStashEntryMarker = "!!CodeMaster" //AuroraEditor

    private let editorStashEntryMessageRe = "/!!CodeMaster<(.+)>$/" //AuroraEditor

    public class StashResult {
        /// The stash entries created by Desktop
        var aeEntries: [StashEntry]
        /// The total amount of stash entries,
        /// i.e. stash entries created both by CM and outside of CM
    ...
    /// Returns the last CM created stash entry for the given branch
    ...
    /// Creates a stash entry message that indicates the entry was created by CodeMaster
    public func createCMStashMessage(branchName: String) -> String {
    ...
    public func createAEStashEntry(directoryURL: URL,
        ...
        let branchName = branch
        let message = createCMStashMessage(branchName: branchName)
